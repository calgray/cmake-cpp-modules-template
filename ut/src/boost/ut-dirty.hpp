...
//
#if defined(__cpp_modules) && not defined(BOOST_UT_DISABLE_MODULE)
export module boost.ut;
// export import stdcxx;
#define STR_CONSTEXPR constexpr
#else // defined(__cpp_modules) && defined(BOOST_UT_DISABLE_MODULE)
#pragma once
#define STR_CONSTEXPR
#endif // defined(__cpp_modules) && defined(BOOST_UT_DISABLE_MODULE)

#if __has_include(<iso646.h>)
#include <iso646.h>  // and, or, not, ...
#endif // __has_include(<iso646.h>)
...
#endif // not defined(__cpp_rvalue_references)
...
#endif // defined(__cpp_modules) && not defined(BOOST_UT_DISABLE_MODULE)
...
export namespace boost::inline ext::ut::inline v1_1_9 {
#else
...
constexpr const std::size_t raw_length = raw_type_name.length();
constexpr const std::string_view need_name =
    ...
    "unique_name_for_auto_detect_prefix_and_suffix_lenght_0123456789_struct";
#endif
constexpr const std::size_t need_length = need_name.length();
// static_assert(need_length <= raw_length,
//               "Auto find prefix and suffix lenght broken error 1");
constexpr const std::size_t prefix_length =
...
constexpr const std::size_t tail_lenght = raw_length - prefix_length;
// static_assert(need_length <= tail_lenght,
//               "Auto find prefix and suffix lenght broken error 4");
constexpr const std::size_t suffix_length = tail_lenght - need_length;
...
template <class T>
constexpr auto has_user_print = is_valid<T>(
 ...
 public:
  ...
  auto on(events::test<Ts...> test) {
    ...
    if (!detail::cfg::query_pattern.empty()) {  //
      const std::regex regex(detail::cfg::query_regex_pattern);
...
#endif // defined(__cpp_nontype_template_parameter_class)
...
#else // defined(__GNUC__) || defined(__clang__) || defined(__INTEL_COMPILER)
